...
namespace AppointmentBot
{
    public class TwilioAdapterWithErrorHandler : TwilioAdapter
    {
        public TwilioAdapterWithErrorHandler(IConfiguration configuration, ILogger<TwilioAdapter> logger)
                : base(configuration, getOptions(), logger)
        {
        ...
        private static TwilioAdapterOptions getOptions()
        {
            TwilioAdapterOptions options = new TwilioAdapterOptions();
            options.ValidateIncomingRequests = false;
            return options;
        }
    }
}
